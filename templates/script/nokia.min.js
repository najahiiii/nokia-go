var NokiaApp=(()=>{var $=(e,t)=>()=>(t||e((t={exports:{}}).exports,t),t.exports);var U=$(()=>{const h={baseUrl:`${window.location.origin}/api`,async fetchData(e,t=""){const n=`${this.baseUrl}/${e}${t?"?"+t:""}`,a=await fetch(n);if(!a.ok){let s=`HTTP error! status: ${a.status}`;try{const i=await a.json();i?.error&&(s=i.error)}catch{}throw new Error(s)}return await a.json()},async sendJSON(e,t,n="POST"){const a=await fetch(`${this.baseUrl}/${e}`,{method:n,headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!a.ok){let s=`HTTP error! status: ${a.status}`;try{const i=await a.json();i?.error&&(s=i.error)}catch{}throw new Error(s)}try{return await a.json()}catch{return{}}},getFormattedDailyUsage(){return this.fetchData("daily_usage")},getPreloginStatus(){return this.fetchData("prelogin_status")},getOverview(){return this.fetchData("overview")},getWanStatus(){return this.fetchData("wan_status")},getDeviceStatus(){return this.fetchData("device_status")},getNetworkClients(){return this.fetchData("network_clients")},getServiceData(){return this.fetchData("service_data")},getStatusWeb(){return this.fetchData("status_web")},getCurrentLedState(){return this.fetchData("led_state")},setLedState(e){return this.sendJSON("led_state",{enable:e})},setApnInternet(e){return this.fetchData("set_apn",`apn=${encodeURIComponent(e)}`)},doReboot(){return this.fetchData("do_reboot")},getLanStatus(){return this.fetchData("lan_status")},getWlan24gStatus(){return this.fetchData("wlan_configs_24g")},getWlan5gStatus(){return this.fetchData("wlan_configs_5g")},getSmsList(){return this.fetchData("sms")},setSmsState(e,t){return this.fetchData("set_sms_state",`smsid=${encodeURIComponent(e)}&smsunread=${encodeURIComponent(t)}`)},deleteSms(e,t={}){const a=(Array.isArray(e)?e:e===void 0?[]:[e]).map(i=>typeof i=="number"?String(i):(i??"").toString()).map(i=>i.trim()).filter(i=>i.length>0),s={};return a.length>0&&(s.sms_ids=a),t.deleteAll&&(s.delete_all=!0),!s.delete_all&&!s.sms_ids?Promise.reject(new Error("No SMS IDs provided")):this.sendJSON("delete_sms",s)},getCellularIdentity(){return this.fetchData("cell_identification")},getDataExpired(){return this.fetchData("get_data_expired")},setDataExpired(e){return this.fetchData("set_data_expired",`data_expired=${encodeURIComponent(e)}`)},getConfig(){return this.fetchData("config")},updateConfig(e){return this.sendJSON("config",e)},checkListener(e,t){const n=new URLSearchParams;return e!=null&&n.set("host",e),t!=null&&n.set("port",t),this.fetchData("config/listener_available",n.toString())}},m={escapeHtml(e){return e==null?"":String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")},updateSignalBars(e){e=Math.max(0,Math.min(5,Math.floor(e))),document.querySelectorAll(".signal-bar").forEach((t,n)=>{const a=n<5-e;t.setAttribute("fill",a?"#666666":"#6495ed"),t.setAttribute("opacity",a?"0.4":"1")})},formatUptime(e){return[{value:86400,singular:"day",plural:"days"},{value:3600,singular:"hr",plural:"hrs"},{value:60,singular:"min",plural:"mins"},{value:1,singular:"sec",plural:"secs"}].reduce((n,a)=>{const s=Math.floor(e/a.value);return(s>0||n.length===0)&&(n.push(`${s} ${s===1?a.singular:a.plural}`),e%=a.value),n},[]).join(" ")},updateProgressBar(e,t,n,a,s=!1){const i=document.getElementById(e);if(!i)return;const r=Math.max(0,Math.min(100,(t-n)/(a-n)*100));i.style.width=`${r}%`;let o;s?o=r>75?"bg-red-500":r>45?"bg-yellow-500":"bg-green-500":o=r>75?"bg-green-500":r>45?"bg-yellow-500":"bg-red-500",i.className=`h-4 rounded-full transition-all duration-300 ${o}`},updateSignalStats(e){if(!e.cell_LTE_stats_cfg?.[0]?.stat)return;const t=e.cell_LTE_stats_cfg[0].stat;this.updateSignalBars(t.RSRPStrengthIndexCurrent),this.setTextContent("radioRSSI",`${t.RSSICurrent} dBm`),this.updateProgressBar("rssiProgress",t.RSSICurrent,-120,-30),this.setTextContent("radioRSRP",`${t.RSRPCurrent} dBm`),this.updateProgressBar("rsrpProgress",t.RSRPCurrent,-140,-64),this.setTextContent("radioSNR",`${t.SNRCurrent} dB`),this.updateProgressBar("sinrProgress",t.SNRCurrent,-10,30),this.setTextContent("radioRSRQ",`${t.RSRQCurrent} dBm`),this.updateProgressBar("rsrqProgress",t.RSRQCurrent,-20,-3)},updateNetworkInfo(e){if(e.ntwtopo_cfg?.[0]&&this.setTextContent("routerIp",e.ntwtopo_cfg[0].IPAddress),e.wan_conns?.[0]?.ipConns?.[0]){const t=e.wan_conns[0].ipConns[0].ExternalIPAddress||"Unavailable";this.setTextContent("wanIp",t),this.setTextContent("wanIpPreview",t)}if(e.wan_conns?.[0]?.ipConns?.[0]){const t=(e.wan_conns[0].ipConns[0].DNSServers||"").split(/[\s,]+/).map(a=>a.trim()).filter(Boolean),n=document.getElementById("dnsServer");n&&(t.length===0?n.innerHTML=`
                    ...
                    `:n.innerHTML=`
                        ...
                        <div class="flex flex-col text-right">
                            ${t.map(a=>`<span class="font-semibold text-xs">${this.escapeHtml(a)}</span>`).join("")}
                        </div>
            ...
            `)}t.insertAdjacentHTML("beforeend",n.join(""))},updateWlanStatus(e){const t=document.getElementById("wlanList");if(t.innerHTML="",e?.wlan_config_glb?.[0]){const n=e.wlan_config_glb[0],a=n.Enable?"checked":"",s=`
                ...
                `;n.insertAdjacentHTML("beforeend",r)})}},updateCellularIdentity(e){this.setTextContent("radio-access-title",e?.FunctionResult?.Name??"Radio Access")},renderDailyUsageChart(e){const t=document.querySelector("#dailyUsageChart");if(!t)return;t.innerHTML="";const n=document.createElementNS("http://www.w3.org/2000/svg","svg");if(n.setAttribute("viewBox","0 0 100 100"),n.setAttribute("class","w-full h-auto max-w-[200px] mx-auto"),e.last_7_days.length===1){const d=e.last_7_days[0],y="#3B82F6",f=document.createElementNS("http://www.w3.org/2000/svg","circle");f.setAttribute("cx","50"),f.setAttribute("cy","50"),f.setAttribute("r","45"),f.setAttribute("fill",y),f.setAttribute("stroke","#444"),f.setAttribute("stroke-width","0.5"),f.setAttribute("class","transition-all duration-300 hover:opacity-80"),f.setAttribute("data-date",d.date),f.setAttribute("data-usage",d.combined.formatted),f.addEventListener("mouseenter",S=>{const E=document.createElement("div");E.className="absolute bg-[#333] p-2 rounded shadow-lg text-xs z-10 border border-gray-600",E.innerHTML=`
                    ...
                    100%
                `,E.style.top=`${S.clientY-60}px`,E.style.left=`${S.clientX}px`,E.id="chart-tooltip",document.body.appendChild(E)}),f.addEventListener("mouseleave",()=>{const S=document.getElementById("chart-tooltip");S&&S.remove()}),n.appendChild(f);const w=document.createElementNS("http://www.w3.org/2000/svg","text");w.setAttribute("x","50"),w.setAttribute("y","50"),w.setAttribute("text-anchor","middle"),w.setAttribute("dominant-baseline","middle"),w.setAttribute("class","text-sm font-bold fill-gray-300"),n.appendChild(w);const v=document.createElement("div");v.className="flex flex-wrap justify-center gap-2 mt-4";const D=document.createElement("div");D.className="flex items-center text-xs";const b=document.createElement("div");b.className="w-3 h-3 rounded-full mr-1",b.style.backgroundColor=y;const B=document.createElement("span");B.className="text-gray-300",B.textContent=new Date(d.date).toLocaleDateString("id-ID",{month:"short",day:"numeric"}),D.appendChild(b),D.appendChild(B),v.appendChild(D),t.appendChild(n),t.appendChild(v);return}const a=e.last_7_days.reduce((d,y)=>d+y.combined.raw_bytes,0);let s=0;const i=["#A29BFE","#FF8C94","#FFB266","#2EE59D","#4FC3F7","#FF6F61","#FFD54F"],r=e.last_7_days.map(d=>d.combined.raw_bytes/a*100||0),o=1,c=r.some(d=>d<o&&d>0),l=c?r.map(d=>Math.max(d,o)):r,p=l.reduce((d,y)=>d+y,0),u=l.map(d=>d/p*100);e.last_7_days.forEach((d,y)=>{const f=u[y],w=r[y],v=document.createElementNS("http://www.w3.org/2000/svg","path"),D=Math.max(f,.1),b=50+45*Math.cos(2*Math.PI*s/100),B=50+45*Math.sin(2*Math.PI*s/100);s+=D;const S=50+45*Math.cos(2*Math.PI*s/100),E=50+45*Math.sin(2*Math.PI*s/100),_=D>50?1:0,M=["M 50 50",`L ${b} ${B}`,`A 45 45 0 ${_} 1 ${S} ${E}`,"Z"].join(" ");v.setAttribute("d",M),v.setAttribute("fill",i[y%i.length]),v.setAttribute("stroke","#444"),v.setAttribute("stroke-width","0.5"),v.setAttribute("class","transition-all duration-300 hover:opacity-80"),v.setAttribute("data-date",d.date),v.setAttribute("data-usage",d.combined.formatted),v.addEventListener("mouseenter",L=>{const I=document.createElement("div");I.className="absolute bg-[#333] p-2 rounded shadow-lg text-xs z-10 border border-gray-600",I.innerHTML=`
                    <strong class="text-white">${d.date}</strong><br>
                    <span class="text-blue-400">\u2191 ${d.upload.formatted}</span><br>
                    <span class="text-green-400">\u2193 ${d.download.formatted}</span><br>
                    <strong class="text-white">Total: ${d.combined.formatted}</strong><br>
                    ${w.toFixed(1)}%${c?"*":""}
                `,I.style.top=`${L.clientY-60}px`,I.style.left=`${L.clientX}px`,I.id="chart-tooltip",document.body.appendChild(I)}),v.addEventListener("mouseleave",()=>{const L=document.getElementById("chart-tooltip");L&&L.remove()}),n.appendChild(v)});const g=document.createElementNS("http://www.w3.org/2000/svg","text");g.setAttribute("x","50"),g.setAttribute("y","50"),g.setAttribute("text-anchor","middle"),g.setAttribute("dominant-baseline","middle"),g.setAttribute("class","text-sm font-bold fill-gray-300"),n.appendChild(g),t.appendChild(n);const x=document.createElement("div");if(x.className="flex flex-wrap justify-center gap-2 mt-4",e.last_7_days.forEach((d,y)=>{const f=document.createElement("div");f.className="flex items-center text-xs";const w=document.createElement("div");w.className="w-3 h-3 rounded-full mr-1",w.style.backgroundColor=i[y%i.length];const v=document.createElement("span");v.className="text-gray-300",v.textContent=new Date(d.date).toLocaleDateString("id-ID",{month:"short",day:"numeric"}),v.textContent+=` (${d.combined.formatted})`,f.appendChild(w),f.appendChild(v),x.appendChild(f)}),c){const d=document.createElement("div");d.className="text-xs text-gray-400 mt-2 text-center",d.textContent="* Small values are exaggerated for visibility",t.appendChild(d)}t.appendChild(x)},updateSms(e){if(!e?.FunctionResult?.SMSList)return;const t=e.FunctionResult.SMSList,n=t.filter(c=>c.SMSUnread).length,a=document.getElementById("smsBadge");n>0?(a.textContent=n,a.classList.remove("hidden")):a.classList.add("hidden");const s=this.smsData||[],i=new Set(s.map(c=>c.SMSID)),r=t.filter(c=>!i.has(c.SMSID));if(this.smsData=t,r.length>0){const c=p=>{const u=p.SMSSender||"Unknown";let x=(p.SMSContent||"").trim();return x.length>80&&(x=`${x.slice(0,77)}\u2026`),x?`${u}: ${x}`:`From ${u}`},l=r.length===1?c(r[0]):`${r.length} new messages`;C.showNotification({title:"New SMS received",message:l,tone:"info"})}document.getElementById("smsDialog")?.classList.contains("active")&&T()}};function A(e,t,{loadingText:n="Processing\u2026",spinnerColor:a="text-white"}={}){if(e)if(t){e.dataset.defaultContent||(e.dataset.defaultContent=e.innerHTML);const s=m.escapeHtml(n);e.disabled=!0,e.setAttribute("aria-busy","true"),e.innerHTML=`
